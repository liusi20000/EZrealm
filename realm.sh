...
deploy_realm() {
    mkdir -p /root/realm
    cd /root/realm
    wget -O realm.tar.gz https://git.8517979.xyz/https://github.com/zhboner/realm/releases/download/v2.7.0/realm-x86_64-unknown-linux-gnu.tar.gz
...
import_rules() {
    ...
    # 逐行处理导入的数据
    while IFS='|' read -r index local_port remote_addr remark || [ -n "$index" ]; do
        # 跳过空行或格式不正确的行
